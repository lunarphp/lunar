...
2. **`2.x/addons/search.mdx`**: new "Pipelines" section documenting `lunar.search.pipelines.request` and `results`, the passable objects, `page()`, and the `meta` fields, with a short custom-stage example; the Typesense vector distance threshold and the Meilisearch hybrid keys, since they are `search` settings that apply with or without the add-on.
