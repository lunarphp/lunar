...
- **Behavioural break**: none on default settings — `prevent_lazy_loading` defaults to `false`, so existing behaviour is preserved. Consumers opt in via `LUNAR_PREVENT_LAZY_LOADING=true` (always on) or `'auto'` (on outside production). A later release will flip the default to `'auto'` once the panel-wide audit (see Open Questions) is complete.
...
- **Should the default be `'auto'` or `false`?** Decided: ship `false`. The hotspot audit (next bullet) turned out larger than expected — surfacing real N+1s across admin pages, Filament selectors, and casts. Shipping `'auto'` before that sweep completes is the same "behavioural ambush" we rejected for the global switch, aimed at downstream consumers instead. Flip to `'auto'` in a later release once the panel is provably clean. Owner: Glenn.
...
- **Hotspot audit scope** — section D lists representative cases; this PR fixed those plus the Collection `breadcrumb` / `ancestors` walks in `ManageProductCollections`, `ManageBrandCollections`, `CollectionResource::getCollectionBreadcrumbs`, `CollectionLimitationRelationManager`, `DiscountTargetSelect`, `CollectionSelect`, `CollectionTreeView`, plus a `Price` cast that lazy-loaded `priceable`. The remaining audit (clicking through every panel page with `LUNAR_PREVENT_LAZY_LOADING=true`) is tracked as a follow-up before the default is flipped to `'auto'`. Owner: implementer.
