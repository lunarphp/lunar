...
- `$notification` — a key from the **sendable-notification registry** (below).
...
### Sendable-notification registry (variant choice)

The admin needs a catalogue of *manually sendable* notifications — distinct from `lunar.orders.notifications`, which maps lifecycle *states* to *automatic* sends. A `CustomerNotifications` manifest (code defaults + override seam, mirroring the `CancelReasons` / `HoldReasons` manifests established in [[0033-reduce-config-surface]]) maps a key ⇒ label ⇒ notification class:

```php
CustomerNotifications::register('order-update', OrderUpdate::class);
```

The action's variant dropdown is `CustomerNotifications::all()`. Default entries are populated by the "ship default customer notifications" TODO work (order confirmation, shipping update, …); until those land the registry is empty and the admin action is hidden.
...
- **Reuse `lunar.orders.notifications` as the variant list.** Rejected: that map is keyed by lifecycle *state* for *automatic* sends; the manual catalogue is a different set (an admin sends "delay apology", never "paid"). Conflating them overloads one config with two meanings.
...
- **Additive public surface:** `Contracts\Actions\Orders\NotifiesCustomer`, `Actions\Orders\NotifyCustomer`, `Order::notifyCustomer()` (+ the model contract), `Events\Orders\OrderCustomerNotified`, the `CustomerNotifications` manifest + facade, an optional `AcceptsCustomerMessage` notification contract, and `Filament\Actions\Orders\NotifyCustomerAction`. No breaking change.
...
- [[0033-reduce-config-surface]] — the `CancelReasons` / `HoldReasons` manifest pattern the `CustomerNotifications` registry mirrors.
