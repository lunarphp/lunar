...
- **Filament / admin impact:** `AdjustStockAction` becomes location-aware; the variant edit screen shows per-location buckets, the global rollup, a read-only movement history relation, and active reservations. Supersedes the single-line stock action from [[0009-filament-actions-and-global-search]].
