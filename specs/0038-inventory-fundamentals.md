...
- [ ] **Slice 4 — variant inventory controls (minimal + swappable).** Consolidate the variant Inventory page into one bridge schema: selling-policy fields, the `AdjustStock` action, and a read-only rollup summary (on-hand / committed / reserved / available) + recent movements. No standalone screens. Changeable via extension hooks + subclass-and-rebind; a `LunarPanel` toggle disables it so an add-on can replace it. Per-location editing, full ledger history and reservation management are deliberately left to add-ons.
