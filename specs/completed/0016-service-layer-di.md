...
- **Verification.** Each test suite passes standalone (core 607, filament 40, admin 188, upgrade 29, stripe 40). The full-`pest` run shows 20 pre-existing cross-suite DB-pollution failures (identical count on the base `2.x` branch); this spec adds 2 passing tests and zero new failures. phpstan level 0 clean; pint clean on all changed files.
