...
- [x] Slice 1 — enum + rename + logic. `Enums\SellingPolicy`; rename the column in `create_product_variants_table`; cast on `ProductVariant`; rewrite `getTotalInventory()` / `canBeFulfilledAtQuantity()` to the `match` (fixing the `always`-adds-backorder bug); update factories/tests, standardising the `in_stock_or_backorder` fixture typo.
- [x] Slice 2 — Filament + locales. Form field key/label -> `selling_policy` / "Selling Policy" in both packages (`getSellingPolicyComponent()`); live backorder dependency updated; translation keys renamed and values actually translated across 16 locales.
- [x] Slice 3 — upgrade package. Column rename + total value-reconciliation data migration (typo corrected, recognised values passed through, unknowns -> `always`; one-way); class-scoped `RenamePropertyRector` entry for `ProductVariant` (with a fixture test proving the morph/pivot stay untouched); upgrade notes flagging the `always` inventory behaviour change and untyped-access manual-review caveat.
