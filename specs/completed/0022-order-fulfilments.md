...
> **Implementation amendment (2026-06-17).** Upgrade-path deltas recorded after the body below was written; later specs refined the named seams (the steps still hold):
> - **`OrderStatusUpdated` event removed** with the headline state — it carried an `OrderState` that no longer exists. Split listeners across the survivors: `OrderPaymentStatusUpdated` / `OrderFulfilmentStatusUpdated` (rollups), `OrderClosed` / `OrderReopened` (lifecycle) and `OrderCancelled` ([[0025-order-cancellation]]). No Rector target — one event became several, each with a different payload; manual migration.
> - **Search re-index required.** `OrderIndexer` drops the `status` filterable attribute and adds `payment_status` + `fulfilment_status` + `closed` (filterable) and `closed_at` (sortable). After upgrading, run `php artisan scout:import` for the order model and update the engine's `filterableAttributes` / `sortableAttributes`; move any filter off the old `status` attribute to the new columns.
> - **Superseded seams.** The `FulfilmentManager` / `Facades\Fulfilments` surface listed below is retired in favour of model verbs (`$fulfilment->ship()`, `$order->cancel()`) — [[0029-entry-point-conventions]]; `OrderStateConfig` becomes `OrderSettings`, with per-store reasons/auto-close moved off config — [[0033-reduce-config-surface]].
