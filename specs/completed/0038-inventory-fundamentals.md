...
- [x] **Slice 4b — control rework.** `ProductVariantInventory` bridge schema (via the `configureInventory` hook + subclass-and-rebind): read-only rollup summary (on-hand / available / committed / reserved), selling-policy fields, recent movements; the `AdjustStock` action sits in the Stock section header and the editable stock field is gone (stock is ledger-derived). The Stock card names the default location it manages (where adjustments record). `getStockComponent()` removed. Per-location editing, full ledger history and reservation management left to add-ons.
- [x] **Slice 5 — upgrade package.** A self-sufficient v1 → v2 data migration: adds the rollup columns, ensures the stock tables, backfills a `StockLevel` + `OpeningBalance` movement at the default location from each v1 variant's `stock`, seeds the rollup, then drops `stock`. Guarded/idempotent, no `down()`. `backorder`/`purchasable` untouched. No `AdjustStock` Rector needed (the `Location` parameter is nullable-last). The new stock tables are added to the ledger `v2_baseline`.
