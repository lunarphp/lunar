...
- `Models\Concerns\HasModelExtending` in full — `fireModelEvent` re-dispatch, `newModelQuery`/`replicateInto`, `__callStatic`, the `getTable`/`getForeignKey`/`getMorphClass` parent-walking, `modelClass()`, `morphName()` and `isLunarInstance()`.
...
- [ ] Slice 2 — Collapse every internal `X::modelClass()` call site (relations, morph-map registration, Filament resource/relation resolution) to the concrete Lunar class, and retype any public signatures that name a `Models\Contracts\*` interface. Mechanical, Rector-assisted; leaves nothing depending on the extending machinery.
- [ ] Slice 3 — Delete `HasModelExtending`, the `Models\Contracts\*` interfaces, and `ModelManifest`'s substitution API (`replace`/`add`/`get`/`guessContractClass`); bind the morph map directly to concrete classes. Delete the `extend-models` CI dimension and the `tests/core/Stubs/Models` substitution stubs. The slice-1 recipe tests must stay green throughout.
...
- [ ] Slice 5 — `upgrade` package: Rector rules and migration guide mapping each former extending use to its native replacement, cross-checked against the slice-1/4 recipe tests so the guide and the tests cannot drift.
