...
- [ ] Slice 2b — Remove the `Models\Contracts\*` interfaces and retype every signature that names them to the concrete class.
- [ ] Slice 3 — Delete `HasModelExtending` and `ModelManifest`'s substitution API (`replace`/`add`/`get`/`guessContractClass`); bind the morph map directly to concrete classes. Delete the `extend-models` CI dimension and the `tests/core/Stubs/Models` substitution stubs. The slice-1 recipe tests must stay green throughout.
...
- [ ] Slice 5 — `upgrade` package: Rector rules (including the consumer-facing `Class::modelClass()` -> `Class::class` rewrite, tested with fixtures) and a migration guide mapping each former extending use to its native replacement, cross-checked against the slice-1/4 recipe tests so the guide and the tests cannot drift.
