...
- **Customer** — `CustomerCreated`, `CustomerUpdated`, `CustomerDeleted`. (The admin-only `CustomerAddressEdited` retires — addresses aren't indexed; `CustomerUserEdited` stays, since a linked user's email is in the customer search document.)
