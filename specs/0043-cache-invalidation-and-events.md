...
- [ ] Slice 3b — Pivot tier + admin retirement. Collection-membership / channel / customer-group / media attach-detach fire no model event, so record at a core verb/seam (added where the operation lived only in Filament); remove the admin/filament `*Updated` events and point relation managers at the core verbs. (Breaking, cross-package, adds public verbs — its own pass.)
