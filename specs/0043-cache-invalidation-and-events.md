...
Scout's own model observer already reindexes a searchable model on its direct save/delete; the gap is the **cascade** (a variant, price, or pivot changes without the parent saving), which is exactly the `RelatedChanged` reason. So a single core listener, `Listeners\ReindexOnCacheInvalidation`, reindexes the invalidated model on `RelatedChanged` only — no double-reindex with Scout, and the cascade now reindexes for programmatic changes too, not just the admin UI. The catalog entries are dropped from the admin `sync_with_search` listener block; customer reindexing stays there until the customer lifecycle events land (Part B). The now-orphaned admin/filament catalog `*Updated` event classes and their dispatch sites are deleted in the Part B sweep, alongside the customer events.
...
- **Cart** — `CartCreated`, `CartDeleted`, `CartMerged`. (`CartLinesUpdated` is deferred: the recalculation pipeline saves lines repeatedly, so a clean, non-noisy dispatch point needs a concrete consumer to design against.)
...
- [ ] Slice 5b — Admin event sweep (breaking). Delete the orphaned catalog `*Updated` event classes (admin + filament) + their dispatch sites; retire the admin-only customer events and move customer reindexing onto a customer cache-invalidation cascade (Customer participates, Address as satellite) so the address/user reindex case survives; upgrade note maps removed events to the core ones.
