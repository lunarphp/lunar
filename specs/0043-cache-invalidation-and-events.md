...
- [ ] Slice 5 — Part B lifecycle events + admin event sweep. Customer, cart, and discount events dispatched from their verbs/observers; retire the admin-only customer events and delete the orphaned catalog `*Updated` event classes + dispatch sites; move customer reindexing onto the new customer events. Breaking (admin-layer surface); upgrade note maps removed events to the core ones.
