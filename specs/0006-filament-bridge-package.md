...
- `packages/filament/composer.json` — `lunarphp/filament`, requires `lunarphp/core: self.version`, `filament/filament: ^5.0`, plus the Filament plugin packages the moved code uses (`filament/spatie-laravel-media-library-plugin`, `mallardduck/blade-lucide-icons`). The originally planned chart plugin (`leandrocfe/filament-apex-charts`) was dropped during implementation in favour of Filament v5's native chart widgets — every dashboard widget under `Widgets/Dashboard/Orders/` now extends `Filament\Widgets\ChartWidget` directly.
