...
const { state, collectOption, collectAvailable, collectionPointRequired, selectCollectionPoint } = useCheckout()
</script>
...
<template>
  <template v-if="collectAvailable">
    ...
    <section class="block" data-block="collect">
      ...
      <div v-if="!state.collectionPoints.length" class="locked">
      ...
      <div v-else-if="state.collectionPoints.length === 1" class="locked">
        <span class="ico"><Icon name="store" :size="17" /></span>
        <span>
          <strong>{{ state.collectionPoints[0].name }}</strong>
          <span v-for="line in state.collectionPoints[0].lines" :key="line" class="pmeta"> · {{ line }}</span>
        </span>
      </div>
      ...
      <div v-else role="radiogroup" aria-label="Choose where to collect your order">
        <button
          ...
          @click="selectCollectionPoint(point.id)"
        >
        ...
        <p v-if="collectionPointRequired" class="help" role="alert">Choose a branch to continue.</p>
      </div>
    </section>
  </template>
