...
function setFlow(flow){
  ...
  seg.querySelectorAll('.seg-btn').forEach(b=> b.setAttribute('aria-checked', b.dataset.flow===flow));
  ...
  // payment is step 4 in delivery (address + shipping precede it) but step 3 in collect (store only)
  const payNum=document.querySelector('[data-block="payment"] .num'); if(payNum) payNum.textContent = flow==='collect' ? '3' : '4';
...
function fillAddr(a, ids){
  Object.entries(ids).forEach(([k,id])=>{ const el=document.getElementById(id); if(el && a[k]!=null){ el.value=a[k]; el.closest('.fl')?.classList.toggle('is-filled', !!a[k]); window.PhoneField?.refresh(id); } });
}
function clearAddr(ids){
  Object.values(ids).forEach(id=>{ const el=document.getElementById(id); if(el){ el.value=''; el.closest('.fl')?.classList.remove('is-filled'); window.PhoneField?.refresh(id); } });
}
