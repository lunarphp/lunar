...
function summaryHTML(){
  ...
  return `
    ...
    <div class="os-rule"></div>
    <div class="os-lines">
      <div class="os-line subtotal"><span>Subtotal</span><span class="v">${fmt(c.subtotal)}</span></div>
...
function renderPickupPoints(list){
  ...
  wrap.innerHTML = list.map(p=>{
    ...
    return `<div class="pick pick-pickup" role="radio" tabindex="${checked?0:-1}" aria-checked="${checked}" data-pickup="${p.id}">
      <span class="radio"></span>
      <div class="pbody">
        <div class="ptop">
          <span class="pname">${carrierMark(p.carrier)}<span class="pname-t">${p.name}</span></span>
          <span class="pprice">${fmt(CARRIERS[p.carrier].fee)}</span>
        </div>
        <div class="psub">
          <span class="psub-i"><span class="ico"><i data-lucide="${typeIcon}"></i></span>${typeLabel}</span>
          <span class="psep">·</span>
          <span class="psub-i"><span class="ico"><i data-lucide="map-pin"></i></span>${p.dist} mi</span>
          <span class="psep">·</span>
          <span class="psub-i popen"><span class="ico"><i data-lucide="clock"></i></span>${p.hours}</span>
        </div>
      </div>
    </div>`;
  }).join('');
...
function renumberSteps(){
  const blocks=[...document.querySelectorAll('.block')].filter(b=>!b.hidden && b.querySelector('.block-step .num'));
