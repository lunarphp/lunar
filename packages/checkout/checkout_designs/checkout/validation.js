...
function fieldError(input){
  ...
  if(rule==='phone' && window.PhoneField) return window.PhoneField.error(input); // country-aware
  const r = validators[rule] ? validators[rule](input.value) : true;
  return r===true ? true : r;
}
/* completion-time phone check (strict: empty = not valid). Falls back to the
   loose regex before phone.js has wired the field. */
function phoneValid(id){
  if(window.PhoneField) return window.PhoneField.valid(id);
  return validators.phone(val(id))===true;
}
function showError(input,msg){
  const fl = input.closest('.fl, .card-group, .phone-group'); if(fl) fl.classList.add('has-error');
...
function clearError(input){
  const fl = input.closest('.fl, .card-group, .phone-group'); if(fl) fl.classList.remove('has-error');
...
function updateCompletion(){
  const emailOk = validators.email(val('email'))===true;
  // in collect, the contact step now also holds the collector name + phone
  const collectContactOk = ['c-first','c-last'].every(id=>val(id)) && phoneValid('c-phone');
  setDone('contact', emailOk && (state.fulfilment!=='collect' || collectContactOk));

  let addrDone;
  if(state.shippingId==='pickup'){
    // pickup mode: recipient name + phone + a chosen point (no street address)
    addrDone = val('first') && val('last') && phoneValid('phone') && !!state.pickupPoint;
  } else {
...
function isComplete(){
  ...
  if(state.fulfilment==='delivery'){
    if(state.shippingId==='pickup'){
      if(!(val('first') && val('last') && phoneValid('phone'))) return false;
    ...
    if(!(val('c-first') && val('c-last') && phoneValid('c-phone'))) return false;
...
function updatePayDisabled(){
  ...
  const hint=document.getElementById('cta-hint'); if(hint) hint.hidden=ok;
}
