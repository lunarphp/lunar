...
class StripePaymentType extends AbstractPayment implements CreatesPaymentIntents, SupportsPaymentHolds, SupportsPaymentIntents, SyncsPaymentIntents
{
    ...
    public function fetchIntent(string $reference): PaymentIntentStatus
    {
        try {
            ...
            // Includes an unrecognised reference: the contract is explicit
            // that a reference the application stored itself going missing is
            // evidence something is wrong, not an answer.
            throw new PaymentIntentException(
                "Could not fetch Stripe payment intent [{$reference}].", previous: $e,
            );
        }
    ...
    public function describeHold(string $reference): ?HoldDescription
    {
        try {
            ...
            // The reference came from the client, so "never heard of it" is a
            // routine answer to an unverified claim.
            return null;
        } catch (ApiErrorException $e) {
            throw new PaymentIntentException(
                "Could not describe Stripe hold [{$reference}].", previous: $e,
            );
        }
    ...
    public function adjustHold(string $reference, int $amountMinor): HoldAdjustment
    {
        try {
            ...
            throw new PaymentIntentException(
                "Could not read Stripe hold [{$reference}] to adjust it.", previous: $e,
            );
        }
        ...
        try {
            ...
            // Not a refusal, so not NeedsReauthorization: the contract draws
            // that distinction explicitly.
            throw new PaymentIntentException(
                "Could not confirm the outcome of adjusting Stripe hold [{$reference}].", previous: $e,
            );
        }
    ...
    public function captureHold(string $reference, int $amountMinor): void
    {
        try {
            ...
            // An unknown outcome is not a capture.
            throw new PaymentIntentException(
                "Could not capture Stripe payment intent [{$reference}].", previous: $e,
            );
        }
    }
    ...
    public function voidIntent(string $reference): void
    {
        try {
            ...
            // An unconfirmed void must never read as voided.
            throw new PaymentIntentException(
                "Could not void Stripe payment intent [{$reference}].", previous: $e,
            );
        }
    }
    ...
    public function refundIntent(string $reference, int $amountMinor, string $idempotencyKey): PaymentRefund
    {
        try {
            ...
            // One failure channel: a refund the gateway cannot confirm throws
            // rather than coming back as an unsuccessful PaymentRefund.
            throw new PaymentIntentException(
                "Could not refund Stripe payment intent [{$reference}].", previous: $e,
            );
        }

        // Always success true, and no transaction: the premise of this call is
        // that no order was placed, and transactions.order_id is not nullable.
