<div class="flex-col px-12 space-y-4">
  <div class="flex items-center justify-between">
    <strong class="text-xl font-bold md:text-2xl">
      {{ __('discounts::index.title') }}
    </strong>

    <div class="text-right">
      <x-hub::button tag="a" href="{{ route('hub.products.create') }}">{{ __('discounts::index.create_discount') }}</x-hub::button>
    </div>
  </div>
  ...
  <div class="space-y-4">
    <x-hub::table>
      <x-slot name="head">
        ...
        <x-hub::table.heading>
          {{ __('discounts::index.starts_at') }}
        </x-hub::table.heading>

        <x-hub::table.heading>
          {{ __('discounts::index.ends_at') }}
        </x-hub::table.heading>

        <x-hub::table.heading>
          {{ __('discounts::index.priority') }}
        </x-hub::table.heading>

        <x-hub::table.heading>
          {{ __('discounts::index.stop') }}
        </x-hub::table.heading>
