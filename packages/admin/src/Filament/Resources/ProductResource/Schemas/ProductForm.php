...
namespace Lunar\Admin\Filament\Resources\ProductResource\Schemas;
    ...
    public static function getStatusShouts(): array
    {
        return [
            Shout::make('product-status')
                ->content(__('lunarpanel::product.status.unpublished.content'))
                ->type('info')
                ->hidden(fn (Model $record) => static::isPublished($record)),
            Shout::make('product-customer-groups')
                ->content(__('lunarpanel::product.status.availability.customer_groups'))
                ->type('warning')
            ...
            Shout::make('product-no-default-customer-group')
                ->content(__('lunarpanel::product.status.availability.no_default_customer_group'))
                ->type('warning')
                ->hidden(fn (Model $record) => ! static::isPublished($record)
            ...
            Shout::make('product-hidden-from-guests')
                ->content(__('lunarpanel::product.status.availability.hidden_from_guests'))
                ->type('warning')
                ->hidden(fn (Model $record) => ! static::isPublished($record)
                    || ! static::hasEnabledCustomerGroup($record)
            ...
            Shout::make('product-channels')
                ->content(__('lunarpanel::product.status.availability.channels'))
                ->type('warning')
