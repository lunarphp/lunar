@if ($this->transactions->count())
    <button class="inline-flex items-center px-4 py-2 font-bold transition border border-transparent border-gray-200 rounded hover:bg-white bg-gray-50 hover:border-gray-200"
            type="button"
            wire:click.prevent="$set('showRefund', true)">
        <x-hub::icon ref="rewind"
                     style="solid"
                     class="w-4 mr-2" />
...
<a class="inline-flex items-center px-4 py-2 font-bold transition border border-transparent border-gray-200 rounded hover:bg-white bg-gray-50 hover:border-gray-200"
   href="{{ route('hub.orders.pdf', $order->id) }}"
   target="_blank">
    <x-hub::icon ref="download"
                 style="solid"
                 class="w-4 mr-2" />
...
<div class="relative flex justify-end flex-1"
    ...
    <x-hub::menu handle="order_actions">
        @if ($component->items->count())
            <button class="inline-flex items-center px-4 py-2 font-bold transition border rounded hover:bg-white bg-gray-50"
                    type="button"
            ...
            <div class="absolute right-0 z-50 w-screen max-w-[200px] mt-2 text-sm bg-white border rounded-lg shadow-lg top-full overflow-hidden"
                 role="menu"
