<div>
    <div class="space-y-4">
        ...
        @foreach ($this->users as $user)
            <div wire:key="user_{{ $user->id }}"
                 class="flex items-center justify-between p-4 bg-white border border-white rounded-md shadow dark:bg-gray-800 dark:border-gray-700">
                <div class="flex items-center gap-4">
                    <x-hub::gravatar email="{{ $user->email }}"
                                     class="w-8 h-8 rounded-full" />

                    <div class="flex gap-4 text-sm text-gray-900 dark:text-white">
                    ...
                    <x-hub::dropdown minimal>
                        <x-slot name="options">
                            ...
                            @if (impersonate_link($user))
                                <x-hub::dropdown.link href="{{ impersonate_link($user) }}"
                                                      target="_blank"
                                                      nofollow>
                                    Impersonate
                                </x-hub::dropdown.link>
    ...
    <x-hub::modal.dialog form="removeUser"
                         wire:model="userIdToRemove">
        <x-slot name="title">
            Remove user
        </x-slot>
        ...
        <x-slot name="footer">
            <x-hub::button type="button"
                           wire:click.prevent="$set('userIdToRemove', null)"
                           theme="gray">{{ __('adminhub::global.cancel') }}</x-hub::button>
