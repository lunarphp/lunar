    ...
    <div class="overflow-hidden shadow sm:rounded-md">
        <div class="flex-col px-4 py-5 space-y-4 bg-white sm:p-6">
                ...
                <div class="flex items-center space-x-2">
                    <div>
                        <select wire:change="setCurrency($event.target.value)"
                                class="block w-full py-1 pl-2 pr-8 text-base text-gray-600 bg-gray-100 border-none rounded-md form-select focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                            @foreach ($this->currencies as $c)
                                <option value="{{ $c->id }}"
                                        @if ($currency->id == $c->id) selected @endif>{{ $c->code }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <x-hub::input.group :label="__('adminhub::inputs.tax_class.label')"
                                        for="tax_class">
                        <x-hub::input.select id="tax_class"
                                             wire:model="variant.tax_class_id">
                ...
                <div>
                    <x-hub::input.group :label="__('adminhub::inputs.tax_ref.label')"
                                        :instructions="__('adminhub::inputs.tax_ref.instructions')"
                                        :errors="$errors->get('variant.tax_ref')"
                                        for="unit_quantity">
                        <x-hub::input.text wire:model.defer="variant.tax_ref"
                                           id="tax_ref" />
                    </x-hub::input.group>
                </div>
            </div>
            <div class="space-y-4">
                <div class="grid grid-cols-3 gap-4">
                    <x-hub::input.group :label="__('adminhub::inputs.unit_quantity.label')"
                        ...
                        <x-hub::input.text type="number"
                                           wire:model.defer="variant.unit_quantity"
                                           id="unit_quantity" />
                    </x-hub::input.group>

                    <x-hub::input.group :label="__('adminhub::inputs.base_price_excl_tax.label')"
                                        :instructions="__('adminhub::inputs.base_price_excl_tax.instructions')"
                                        for="basePrice"
                                        :errors="$errors->get('basePrices.*.price')"
                                        required>
                        <x-hub::input.price wire:model.defer="basePrices.{{ $this->currency->code }}.price"
                                            :symbol="$this->currency->format"
                                            :currencyCode="$this->currency->code"
                                            required />
                    </x-hub::input.group>

                    <x-hub::input.group :label="__('adminhub::inputs.compare_at_price_excl_tax.label')"
                        ...
                        <x-hub::input.price wire:model.defer="basePrices.{{ $this->currency->code }}.compare_price"
                                            :symbol="$this->currency->format"
                                            :currencyCode="$this->currency->code"
                ...
                @foreach ($this->customerGroups as $group)
                    <div wire:key="group_price_{{ $group->id }}">
                        <div class="grid items-center grid-cols-2 gap-4">
                            {{ $group->name }}
                            <x-hub::input.group :label="null"
                                                for="customerGroupPrices">
                                <x-hub::input.price wire:model="customerGroupPrices.{{ $group->id }}.{{ $currency->code }}.price"
            ...
            <div class="flex items-center justify-between pt-4 border-t">
                ...
                <x-hub::button :disabled="!$currency->default"
                               wire:click.prevent="addTier"
                               theme="gray"
                               size="sm"
                               type="button">
                ...
                @if (count($tieredPrices))
                    ...
                    <div class="space-y-2">
                        <div class="grid grid-cols-3 gap-4">
                            ...
                            <label class="block text-sm font-medium text-gray-700">
                                {{ __('adminhub::global.lower_limit') }}
                            </label>
                                ...
                                <div wire:key="tier_{{ $index }}">
                                    <div class="flex items-center">
                                        <div class="grid w-full grid-cols-3 gap-4">
                                            ...
                                            <x-hub::input.text id="tier_field_{{ $index }}"
                                                               wire:model.defer='tieredPrices.{{ $index }}.tier'
                                                               type="number"
                                                               min="2"
                                                               steps="1"
                                                               required
                                                               ...
                                                               :disabled="!$this->currency->default"
                                                               :error="$errors->first(
                                                                   'tieredPrices.' . $index . '.tier',
                                                               )" />

                                            <x-hub::input.price wire:model.defer="tieredPrices.{{ $index }}.prices.{{ $currency->code }}.price"
                                                                :symbol="$this->currency->format"
                                                                :currencyCode="$this->currency->code" />
                                        </div>
