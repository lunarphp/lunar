        ...
        <div class="flex-col px-4 py-5 space-y-4 bg-white sm:p-6">
            <div class="space-y-4 md:space-y-0 md:grid md:grid-cols-2 md:gap-4">
                <x-hub::input.group :label="__('adminhub::inputs.name')"
                                    for="name"
                                    :error="$errors->first('currency.name')">
                    <x-hub::input.text wire:model.defer="currency.name"
                                       name="name"
                                       id="name"
                                       :error="$errors->first('currency.name')" />
                </x-hub::input.group>
                ...
                <x-hub::input.group :label="__('adminhub::inputs.code')"
                                    for="code"
                                    :error="$errors->first('currency.code')">
                    <x-hub::input.text wire:model.defer="currency.code"
                                       name="code"
                                       id="code"
                                       :error="$errors->first('currency.code')" />
                </x-hub::input.group>
            ...
            <div class="space-y-4 md:space-y-0 md:grid sm:grid-cols-2 lg:grid-cols-5 md:gap-4">
                <x-hub::input.group :label="__('adminhub::inputs.exchange_rate')"
                                    for="exchange_rate"
                                    :error="$errors->first('currency.exchange_rate')">
                ...
                <x-hub::input.group :label="__('adminhub::inputs.decimal_places')"
                                    for="decimal_places"
                                    :error="$errors->first('currency.decimal_places')">
                    <x-hub::input.text :disabled="$currency->id && !Auth::user()->admin"
                                       wire:model.defer="currency.decimal_places"
                                       name="decimal_places"
                                       id="decimal_places"
                                       :error="$errors->first('currency.decimal_places')" />
                </x-hub::input.group>
            ...
            <div class="grid-cols-2 space-y-4 md:space-y-0 md:grid md:gap-4">
                <x-hub::input.group :label="__('adminhub::inputs.default.label')"
                    ...
                    <x-hub::input.toggle wire:click="toggleDefault"
                                         ...
                                         name="handle"
                                         id="handle"
                                         :disabled="$currency->id && $currency->getOriginal('default')" />
                </x-hub::input.group>

                <x-hub::input.group :label="__('adminhub::inputs.enabled.label')"
                                    for="enabled">
                    <x-hub::input.toggle wire:click="toggleEnabled"
                                         :on="$currency->enabled"
                                         name="enabled"
                                         id="enabled"
            ...
            <div class="p-6 space-y-4 text-sm">
                <div class="grid grid-cols-12 gap-4">
                    <div class="col-span-12 md:col-span-6">
                        <strong>
                            {{ __('adminhub::inputs.danger_zone.label', [
                                'model' => 'Currency',
                            ]) }}
                        </strong>
