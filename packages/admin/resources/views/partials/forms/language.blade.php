  ...
  <div class="overflow-hidden shadow sm:rounded-md">
    <div class="flex-col px-4 py-5 space-y-4 bg-white sm:p-6">
      <div class="space-y-4 md:space-y-0 md:grid md:grid-cols-2 md:gap-4">
        <x-hub::input.group :label="__('adminhub::inputs.name')" for="name" :error="$errors->first('language.name')">
          <x-hub::input.text wire:model="language.name" name="name" id="name" :error="$errors->first('language.name')" />
        </x-hub::input.group>
        <x-hub::input.group :label="__('adminhub::inputs.code')" for="code" :error="$errors->first('language.code')">
          <x-hub::input.text wire:model="language.code" name="code" id="code" :error="$errors->first('language.code')" />
        </x-hub::input.group>
      </div>
      <x-hub::input.group
        label="Default"
      ...
      >
        <x-hub::input.toggle wire:click="toggleDefault" :on="$language->default" name="handle" id="handle" :disabled="$language->id && $language->getOriginal('default')" />
      </x-hub::input.group>
    </div>
