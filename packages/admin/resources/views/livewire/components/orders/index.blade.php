    ...
    <x-hub::modal.dialog form="updateStatus"
        ...
        <x-slot name="content">
            <x-hub::input.group :label="__('adminhub::inputs.status.label')"
                ...
                <x-hub::input.select wire:model.defer="status"
                                     required>
                    <option value>
            ...
            <x-slot name="toolbar">
                <div class="p-4 space-y-4 border-b"
                    ...
                    <div class="grid grid-cols-4 gap-4"
                            ...
                            <x-hub::input.group :label="$filter->heading"
                                ...
                                <x-hub::input.select wire:model="filters.{{ $filter->field }}">
                                    ...
                                    @foreach ($this->orders->facets->get($filter->field) as $facet)
                                        <option value="{{ $facet->value }}">{{ $filter->format($facet->value) }}
                                        </option>
                                    @endforeach
                                </x-hub::input.select>
                            </x-hub::input.group>
                        ...
                        <x-hub::input.group :label="__('adminhub::inputs.from_date.label')"
                                            for="from_date">
                            <x-hub::input.datepicker wire:model="filters.from" />
                        </x-hub::input.group>

                        <x-hub::input.group :label="__('adminhub::inputs.to_date.label')"
                                            for="to_date">
            ...
            <x-slot name="body">
                @forelse($this->orders->items as $order)
                    <x-hub::table.row wire:key="row-{{ $order->id }}"
                                      :selected="in_array($order->id, $selected)">
                        <x-hub::table.cell>
