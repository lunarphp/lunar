<x-hub::slideover :title="__(
    $attribute->id
        ? 'adminhub::components.attribute-edit.update_title'
        : 'adminhub::components.attribute-edit.create_title',
)"
    ...
    <div class="space-y-4">
        ...
        <x-hub::input.group :label="__('adminhub::inputs.name')"
            ...
            <x-hub::translatable>
                <x-hub::input.text wire:model.defer="attribute.name.{{ $this->defaultLanguage->code }}"
                                   :error="$errors->first('attribute.name.' . $this->defaultLanguage->code)"
                ...
                @foreach ($this->languages->filter(fn($lang) => !$lang->default) as $language)
                    <x-slot :name="$language['code']">
                        <x-hub::input.text wire:model.defer="attribute.name.{{ $language->code }}"
                                           :placeholder="__('adminhub::components.attribute-edit.name.placeholder')" />
                    </x-slot>
                @endforeach
            </x-hub::translatable>
        ...
        <x-hub::input.group :label="__('adminhub::inputs.handle')"
            ...
            <x-hub::input.text id="handle"
                               wire:model.defer="attribute.handle"
        ...
        <div class="grid grid-cols-3">
            <x-hub::input.group :label="__('adminhub::inputs.required')"
                                for="required"
                                :error="$errors->first('attribute.required')"
                                :instructions="__('adminhub::components.attribute-edit.required.instructions')">
                <x-hub::input.toggle :disabled="!!$attribute->system"
                                     id="required"
                                     wire:model="attribute.required"
                                     value="1" />
            </x-hub::input.group>

            <x-hub::input.group :label="__('adminhub::inputs.searchable.label')"
                                for="searchable"
                                :error="$errors->first('attribute.searchable')"
                                :instructions="__('adminhub::components.attribute-edit.searchable.instructions')">
                <x-hub::input.toggle id="searchable"
                                     wire:model="attribute.searchable" />
            </x-hub::input.group>

            <x-hub::input.group :label="__('adminhub::inputs.filterable.label')"
                                for="filterable"
                                :error="$errors->first('attribute.filterable')"
                                :instructions="__('adminhub::components.attribute-edit.filterable.instructions')">
                <x-hub::input.toggle :disabled="!!$attribute->system"
                                     id="filterable"
                                     wire:model="attribute.filterable" />
            </x-hub::input.group>
        </div>

        <x-hub::input.group :label="__('adminhub::inputs.validation_rules.label')"
                            for="handle"
                            :error="$errors->first('attribute.validation_rules')"
                            :instructions="__('adminhub::components.attribute-edit.validation.instructions')">
            <x-hub::input.text :disabled="!!$attribute->system"
                               id="validation_rules"
                               wire:model.defer="attribute.validation_rules" />
        </x-hub::input.group>

        <x-hub::input.group :label="__('adminhub::inputs.type.label')"
                            for="handle"
    ...
    <x-slot name="footer">
        <div class="flex justify-between">
            <x-hub::button theme="gray"
                           type="button"
                           wire:click="$set('panelVisible', false)">
                {{ __('adminhub::components.attribute-edit.cancel_btn') }}</x-hub::button>
            <x-hub::button wire:click="save">{{ __('adminhub::components.attribute-edit.save_attribute_btn') }}
            </x-hub::button>
        </div>
    </x-slot>
</x-hub::slideover>
