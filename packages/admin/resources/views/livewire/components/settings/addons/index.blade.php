<div class="space-y-4">
    <header>
        <h1 class="text-xl font-bold text-gray-900 md:text-2xl dark:text-white">
            {{ __('adminhub::settings.addons.index.title') }}
        </h1>
    </header>
            ...
            @foreach ($addons as $addon)
                <x-hub::table.row wire:loading.class.delay="opacity-50">
                    ...
                    <x-hub::table.cell :class="$addon['marketplaceId'] ? 'text-green-500' : 'text-red-500'">
                        <x-hub::icon :ref="$addon['marketplaceId'] ? 'check' : 'x'"
                                     class="w-4"
                                     style="solid" />
                    </x-hub::table.cell>
                    <x-hub::table.cell :class="$addon['licensed'] ? 'text-green-500' : 'text-red-500'">
                        <x-hub::icon :ref="$addon['licensed'] ? 'check' : 'x'"
                                     class="w-4"
                                     style="solid" />
                    </x-hub::table.cell>
                    <x-hub::table.cell>
                    ...
                    <x-hub::table.cell>
                        <a href="{{ route('hub.addons.show', $addon['marketplaceId']) }}"
