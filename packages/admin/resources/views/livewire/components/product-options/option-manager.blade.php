          ...
          <div class="flex flex-wrap items-center gap-2">
              @foreach($option->values as $value)
                <label class="relative block" wire:key="option_{{ $value->id }}">
                  ...
                  <span class="items-center inline-block px-2 py-1 pl-6 space-x-1 text-sm border rounded shadow-sm cursor-pointer hover:bg-gray-100 bg-gray-50 peer-checked:bg-sky-200 peer-checked:text-sky-900 peer-checked:border-sky-500">{{ $value->translate('name') }}</span>
                </label>
              @endforeach
          </div>
        </div>
