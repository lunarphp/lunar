            ...
            <div class="p-4 space-y-4 border-b"
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <div class="flex items-center w-full space-x-4">
                        <x-hub::input.text placeholder="Search by attribute or SKU"
                                           class="py-2"
                                           wire:model="search" />
                ...
                <div class="grid grid-cols-4 gap-4"
                     ...
                     x-cloak>
                    <x-hub::input.group :label="__('adminhub::global.status')"
                                        for="brand">
                    ...
                    <x-hub::input.group :label="__('adminhub::global.show_deleted')"
                                        for="shoDeleted">
        ...
        <x-slot name="body">
            @if ($selectPage)
                <x-hub::table.row class="border-b bg-indigo-50">
                    <x-hub::table.cell colspan="24">
                        @unless($selectAll)
                            <span
                                  class="text-sm text-indigo-800">{{ __('adminhub::components.products.index.selected_products', [
                        ...
                        @else
                            <span class="text-sm text-indigo-800">
                                {{ __('adminhub::components.products.index.you_have_selected_all', [
            ...
            @forelse($products as $product)
                <x-hub::table.row wire:key="row-{{ $product->id }}">
                    <x-hub::table.cell>
                        <x-hub::input.checkbox wire:model="selected"
                                               :value="$product->id" />
                    </x-hub::table.cell>
                    ...
                    <x-hub::table.cell>
                        @if ($product->variants->count() > 2)
                            <x-hub::tooltip text="{{ $product->variants->pluck('sku')->join(', ') }}"
                                            left>
                                Multiple
                            </x-hub::tooltip>
                        @else
                    ...
                    <x-hub::table.cell>
                        ...
                        @else
                            <a href="{{ route('hub.products.show', $product->id) }}"
