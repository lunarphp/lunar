...
const breadcrumbs: BreadcrumbItem[] = [
    { label: t('nav.catalog') },
    { label: t('nav.product_types'), current: true },
];
...
const props = defineProps<{
    productTypes: Paginated<ProductTypeRow>;
...
const sortOptions: { value: string; label: string; sort: string; direction: string }[] = [
    { value: 'recent', label: t('product-types.sort_recent'), sort: 'created_at', direction: 'desc' },
    { value: 'oldest', label: t('product-types.sort_oldest'), sort: 'created_at', direction: 'asc' },
    { value: 'name', label: t('product-types.sort_name'), sort: 'name', direction: 'asc' },
    { value: 'products', label: t('product-types.sort_products'), sort: 'products_count', direction: 'desc' },
];
...
const statusOptions: FilterOption[] = [
    { value: 'all', label: t('product-types.filter_all_statuses') },
    { value: 'active', label: t('product-types.status_active') },
    { value: 'draft', label: t('product-types.status_draft') },
];
...
<template>
    <PanelLayout>
        <div data-screen-label="Product types" class="contents">
            ...
            <PageHeader
                :title="t('product-types.title')"
                :description="t('product-types.description')"
                icon="boxes"
            >
                <template #actions>
                    <Link :href="urls.create">
                        <Button variant="primary" icon="plus">{{ t('product-types.new_product_type') }}</Button>
                    </Link>
                </template>
            </PageHeader>
            ...
            <div class="px-4 sm:px-5 lg:px-7 max-w-[1400px] w-full mx-auto pt-5 pb-7">
                ...
                <div class="flex flex-wrap items-center gap-2 mb-4 min-h-[34px]">
                    <template v-if="!(hasBulkActions && selected.length)">
                        <div class="flex-1 max-w-[280px] min-w-[180px]">
                            <TextInput v-model="q" clearable :placeholder="t('product-types.search_placeholder')">
                                <template #prefix><Icon name="search" cls="sm" /></template>
                            </TextInput>
                        </div>
                        <FilterDropdown v-model="statusFilter" :label="t('product-types.filter_status')" :options="statusOptions" default-value="all" />
                        ...
                        <button
                            ...
                            @click="clearFilters"
                        >{{ t('product-types.clear_filters') }}</button>
                        <div class="flex-1" />
                        <span class="text-[11.5px] text-ink-500 whitespace-nowrap">{{ t('product-types.count_of', { shown: productTypes.total, total: totalCount }) }}</span>
                ...
                >
                    <template #empty>
                        <PageEmpty :title="hasActiveFilters ? t('product-types.empty_title') : t('product-types.empty_none_title')">
                            {{ hasActiveFilters ? t('product-types.empty_description') : t('product-types.empty_none_description') }}
                            <div v-if="hasActiveFilters" class="mt-3">
                                <Button @click="clearFilters">{{ t('product-types.clear_filters') }}</Button>
                            </div>
                        </PageEmpty>
                    </template>
                    ...
                    <template #cell-name="{ row }">
                        <div class="min-w-0 flex items-center gap-2.5">
                            <div class="w-8 h-8 rounded-md shrink-0 border border-line grid place-items-center bg-surface-2 text-ink-700">
                                <Icon name="boxes" cls="sm" />
                            </div>
                            <div class="min-w-0">
                    ...
                    <template #cell-attributes_count="{ row }">
                        <span class="text-[12.5px] text-ink-900 font-medium [font-variant-numeric:tabular-nums]">
                            {{ t('product-types.attributes_summary', { product: row.product_attributes_count, variant: row.variant_attributes_count }) }}
                        </span>
                    </template>
