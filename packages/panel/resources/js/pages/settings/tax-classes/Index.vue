...
type TaxClassColumn = { key: string; label: string; width?: string; align?: 'left' | 'right' | 'center' };
...
const props = defineProps<{
    taxClasses: Paginated<TaxClass>;
    columns: TaxClassColumn[];
...
const rowTo = (row: Record<string, unknown>): string => (row as unknown as TaxClass).urls.edit;
...
<template>
    <SettingsShell :title="t('tax_classes.title')" :description="t('tax_classes.description')" wide>
        <template #actions>
            <Button variant="primary" icon="plus" size="sm" @click="openCreate">{{ t('tax_classes.create_tax_class') }}</Button>
        </template>
        ...
        >
            <template #cell-name="{ row }">
                <span class="text-[12.5px] text-ink-900 font-medium">{{ (row as unknown as TaxClass).name }}</span>
                <StatusBadge v-if="(row as unknown as TaxClass).default" tone="sage" size="sm" class="ml-2">{{ t('tax_classes.default_badge') }}</StatusBadge>
            </template>
            <template #cell-variants_count="{ row }">
                <span class="text-xs text-ink-700 [font-variant-numeric:tabular-nums]">{{ (row as unknown as TaxClass).variants_count }}</span>
            </template>
            <template #empty>
                <PageEmpty :title="t('tax_classes.empty_title')" />
            </template>
        </DataTable>

        <div class="mt-4">
            <Pagination :meta="taxClasses" />
        </div>
    </SettingsShell>

    <Dialog
        v-model:open="creating"
        :title="t('tax_classes.create_tax_class')"
        :description="t('tax_classes.create_description')"
    >
        <div class="flex flex-col gap-3">
            <div>
                <FieldLabel required>{{ t('tax_classes.field_name') }}</FieldLabel>
                <TextInput v-model="createForm.name" :invalid="!!createForm.errors.name" :placeholder="t('tax_classes.name_placeholder')" />
            ...
            <label class="flex items-center gap-3 cursor-pointer">
                ...
                <span class="text-[12.5px] text-ink-900 font-medium">{{ t('tax_classes.default_tax_class') }}</span>
            </label>
        </div>
        <template #footer>
