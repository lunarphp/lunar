    ...
    <SettingsShell :title="t('product_options.title')" :description="t('product_options.description')" wide>
        ...
        >
            <template #cell-name="{ row }">
                <span class="text-[12.5px] text-ink-900 font-medium">{{ (row as unknown as ProductOption).name }}</span>
                <StatusBadge v-if="(row as unknown as ProductOption).shared" tone="sage" size="sm" class="ml-2">{{ t('product_options.shared_badge') }}</StatusBadge>
            </template>
            <template #cell-handle="{ row }">
                <span class="font-mono text-xs text-ink-700">{{ (row as unknown as ProductOption).handle || '—' }}</span>
            </template>
            <template #cell-values_count="{ row }">
                <span class="text-xs text-ink-700 [font-variant-numeric:tabular-nums]">{{ (row as unknown as ProductOption).values_count }}</span>
            </template>
