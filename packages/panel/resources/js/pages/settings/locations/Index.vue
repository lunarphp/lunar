...
type Location = {
    ...
    handle: string;
    default: boolean;
    stocked_variants_count: number;
    urls: { edit: string };
};

type LocationColumn = { key: string; label: string; width?: string; align?: 'left' | 'right' | 'center' };
...
const rowTo = (row: Record<string, unknown>): string => (row as unknown as Location).urls.edit;
...
<template>
    <SettingsShell :title="t('locations.title')" :description="t('locations.description')" wide>
        <template #actions>
            <Button variant="primary" icon="plus" size="sm" @click="openCreate">{{ t('locations.create_location') }}</Button>
        </template>
        ...
        >
            <template #cell-name="{ row }">
                <span class="text-[12.5px] text-ink-900 font-medium">{{ (row as unknown as Location).name }}</span>
                <StatusBadge v-if="(row as unknown as Location).default" tone="sage" size="sm" class="ml-2">{{ t('locations.default_badge') }}</StatusBadge>
            </template>
            <template #cell-handle="{ row }">
                <span class="font-mono text-xs text-ink-700">{{ (row as unknown as Location).handle }}</span>
            </template>
            <template #cell-stocked_variants_count="{ row }">
                <span class="text-xs text-ink-700 [font-variant-numeric:tabular-nums]">{{ (row as unknown as Location).stocked_variants_count }}</span>
            </template>
            <template #empty>
                <PageEmpty :title="t('locations.empty_title')" />
            </template>
        </DataTable>

        <div class="mt-4">
            <Pagination :meta="locations" />
        </div>
    </SettingsShell>
    ...
    <Dialog
        ...
        :description="t('locations.create_description')"
    >
        <div class="flex flex-col gap-3">
            <div>
                <FieldLabel required>{{ t('locations.field_name') }}</FieldLabel>
                <TextInput v-model="createForm.name" :invalid="!!createForm.errors.name" :placeholder="t('locations.name_placeholder')" />
                <div v-if="createForm.errors.name" class="mt-1 text-[11px] text-danger">{{ createForm.errors.name }}</div>
            </div>
            <div>
                <FieldLabel :hint="t('locations.handle_hint')">{{ t('locations.field_handle') }}</FieldLabel>
                <TextInput v-model="createForm.handle" mono :invalid="!!createForm.errors.handle" :placeholder="t('locations.handle_placeholder')" />
            ...
            <label class="flex items-center gap-3 cursor-pointer">
                ...
                <span class="text-[12.5px] text-ink-900 font-medium">{{ t('locations.default_location') }}</span>
            </label>
        </div>
        <template #footer>
