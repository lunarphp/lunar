...
type CustomerGroupColumn = { key: string; label: string; width?: string; align?: 'left' | 'right' | 'center' };
...
const props = defineProps<{
    customerGroups: Paginated<CustomerGroup>;
    columns: CustomerGroupColumn[];
...
const rowTo = (row: Record<string, unknown>): string => (row as unknown as CustomerGroup).urls.edit;
...
<template>
    <SettingsShell :title="t('customer_groups.title')" :description="t('customer_groups.description')" wide>
        <template #actions>
            <Button variant="primary" icon="plus" size="sm" @click="openCreate">{{ t('customer_groups.create_group') }}</Button>
        </template>
        ...
        >
            <template #cell-name="{ row }">
                <span class="text-[12.5px] text-ink-900 font-medium">{{ (row as unknown as CustomerGroup).name }}</span>
                <StatusBadge v-if="(row as unknown as CustomerGroup).default" tone="sage" size="sm" class="ml-2">{{ t('customer_groups.default_badge') }}</StatusBadge>
            </template>
            <template #cell-handle="{ row }">
                <span class="font-mono text-xs text-ink-700">{{ (row as unknown as CustomerGroup).handle }}</span>
            </template>
            <template #cell-customers_count="{ row }">
                <span class="text-xs text-ink-700 [font-variant-numeric:tabular-nums]">{{ (row as unknown as CustomerGroup).customers_count }}</span>
            </template>
            <template #empty>
                <PageEmpty :title="t('customer_groups.empty_title')" />
            </template>
        </DataTable>

        <div class="mt-4">
            <Pagination :meta="customerGroups" />
        </div>
    </SettingsShell>

    <Dialog
        v-model:open="creating"
        :title="t('customer_groups.create_group')"
        :description="t('customer_groups.create_description')"
    >
        <div class="flex flex-col gap-3">
            <div>
                <FieldLabel required>{{ t('customer_groups.field_name') }}</FieldLabel>
                <TextInput v-model="createForm.name" :invalid="!!createForm.errors.name" :placeholder="t('customer_groups.name_placeholder')" />
                <div v-if="createForm.errors.name" class="mt-1 text-[11px] text-danger">{{ createForm.errors.name }}</div>
            </div>
            <div>
                <FieldLabel :hint="t('customer_groups.handle_hint')">{{ t('customer_groups.field_handle') }}</FieldLabel>
                <TextInput v-model="createForm.handle" mono :invalid="!!createForm.errors.handle" :placeholder="t('customer_groups.handle_placeholder')" />
            ...
            <label class="flex items-center gap-3 cursor-pointer">
                ...
                <span class="text-[12.5px] text-ink-900 font-medium">{{ t('customer_groups.default_group') }}</span>
            </label>
        </div>
        <template #footer>
