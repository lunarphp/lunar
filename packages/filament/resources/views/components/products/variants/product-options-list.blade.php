...
>
      ...
      <div class="grid grid-cols-2 space-x-4">
        <div>
            <div>
              <label class="fi-fo-field-label ml-7">
                <span class="fi-fo-field-label-content">{{ __('lunar-filament::components.product-options-list.name.label') }}</span>
              </label>
              <div class="flex w-full space-x-2 mt-1 items-start">
                ...
                <div class="grow">
                  ...
                  >
                    {{ __(
                        !$item['readonly'] ?
                            'lunar-filament::components.product-options-list.delete-option.label' :
                            'lunar-filament::components.product-options-list.remove-shared-option.label'
                    ) }}
                  </button>
                </div>
              </div>
        ...
        <div class="space-y-1">
          <label class="fi-fo-field-label">
            <span class="fi-fo-field-label-content">{{ __('lunar-filament::components.product-options-list.values.label') }}</span>
          </label>
          <div wire:key="option_values_{{ $itemIndex }}">
            <x-lunar-filament::products.variants.product-option-list-values
  ...
  <x-filament::button color="gray" size="sm" type="button" wire:click.prevent="addRestrictedOption">
    {{ __('lunar-filament::components.product-options-list.add-option.label') }}
  </x-filament::button>
</div>