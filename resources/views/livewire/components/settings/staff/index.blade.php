...
<div class="text-right">
      <x-hub::button tag="a" href="{{ route('hub.staff.create') }}">
  ...
  <div class="grid items-center grid-cols-8 gap-2">
    <div class="col-span-5 md:col-span-6">
      <x-hub::input.text wire:model.debounce.300ms="search" placeholder="{{ __('adminhub::settings.staff.index.search_placeholder') }}" />
    </div>
    ...
    <x-slot name="body">
      @forelse($staff as $staffMember)
        <x-hub::table.row wire:loading.class.delay="opacity-50">
          <x-hub::table.cell>
            <x-hub::gravatar email="{{ $staffMember->email }}" class="w-6 h-6 rounded-full" />
          </x-hub::table.cell>
          <x-hub::table.cell>
            <x-hub::icon :ref="!$staffMember->deleted_at ? 'check' : 'x'" :class="!$staffMember->deleted_at ? 'text-green-500' : 'text-red-500'" style="solid" />
          </x-hub::table.cell>
          ...
          <x-hub::table.cell>
            <a href="{{ route('hub.staff.show', $staffMember->id) }}" class="text-indigo-500 hover:underline">
