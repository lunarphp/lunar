<div class="mx-12 lg:mx-24">
  <div class="flex items-center space-x-4">
    <a
      href="{{ route(
        $collection->parent_id ? 'hub.collections.show' : 'hub.collection-groups.show',
        [
  ...
  <form wire:submit.prevent="save" class="fixed bottom-0 right-0 z-50 p-6 mr-0 text-right bg-white bg-opacity-75 border-t left-64">
    <div class="flex justify-end w-full space-x-6">
      <x-hub::button type="submit">{{ __('adminhub::catalogue.collections.show.save_btn') }}</x-hub::button>
    </div>
  </form>
  ...
  <div class="py-12 pb-24 lg:grid lg:grid-cols-12 lg:gap-x-12">
    <div class="sm:px-6 lg:px-0 lg:col-span-9">
      ...
      <div id="urls">
        ...
        <div class="shadow sm:rounded-md" id="products">
          <div class="flex-col px-4 py-5 space-y-4 bg-white rounded sm:p-6">
            ...
            <div class="grid items-center grid-cols-2 gap-4">
              <x-hub::input.group label="Sort Products" for="sortProducts">
            ...
            <div wire:sort sort.options='{group: "products", method: "sortProducts"}' class="space-y-2">
              @forelse($products as $product)
                <div sort.item="products" sort.id="{{ $product['id'] }}" wire:key="product_{{ $product['id'] }}" class="flex items-center">
                  ...
                  @endif
                  <div class="flex items-center justify-between w-full px-3 py-3 text-sm bg-white border rounded sort-item-element">
                    <div class="flex items-center">
                      @if($product['thumbnail'])
                        <img src="{{ $product['thumbnail'] }}" class="w-6 mr-2 rounded" />
                      @endif
                      <div>
                        {{ $product['name'] }}
                      </div>
                  ...
                  <div class="ml-4">
                    <button type="button" wire:click.prevent="removeProduct('{{ $product['id'] }}')">
                      <x-hub::icon ref="trash" class="w-4 h-4 text-gray-400 hover:text-red-600" />
                    </button>
                  </div>
                </div>
              @empty
        ...
        <div class="shadow sm:rounded-md" id="collections">
          <div class="flex-col px-4 py-5 space-y-4 bg-white rounded sm:p-6">
            <header class="flex items-center justify-between">
              <h3 class="text-lg font-medium leading-6 text-gray-900">
            ...
            <div class="space-y-2">
              @forelse($collection->children as $child)
                <div class="flex items-center justify-between w-full px-3 py-3 text-sm bg-white border rounded">
                    {{ $child->translateAttribute('name') }}
                    <a href="{{ route('hub.collections.show', [
                      'group' => $collection->group,
                      'collection' => $child,
                    ]) }}" class="text-sm text-blue-500 hover:underline">{{ __('adminhub::global.edit') }}</a>
                </div>
              @empty
                <x-hub::alert>
